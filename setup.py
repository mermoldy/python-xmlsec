...
def latest_release_from_github_api(repo):
    ...
    releases = [
        ...
        if r["prerelease"] is False and r["draft"] is False
    ]
    if not releases:
...
def latest_libiconv_release():
    return latest_release_from_html(
        ...
        re.compile("libiconv-(?P<version>.*).tar.gz"),
    )
...
class build_ext(build_ext_orig):
        ...
        if self.static or sys.platform == "win32":
            ...
            if config is None or not config.get("libraries"):
                raise DistutilsError(
                    "Bad or incomplete result returned from pkg-config."
                )
    ...
    def prepare_static_build(self, build_platform):
        ...
        if openssl_tar is None:
            self.info(
                "{:10}: {}".format("OpenSSL", "source tar not found, downloading ...")
            )
            ...
            if self.openssl_version is None:
                ...
                self.info(
                    "{:10}: {}".format(
                        "OpenSSL",
                        "PYXMLSEC_OPENSSL_VERSION unset, downloading latest from {}".format(
                            url
                        ),
                    )
                )
            else:
                url = "https://api.github.com/repos/openssl/openssl/tarball/openssl-{}".format(
                    self.openssl_version
                )
                self.info(
                    "{:10}: {} {}".format("OpenSSL", "version", self.openssl_version)
                )
        ...
        if zlib_tar is None:
            ...
            if self.zlib_version is None:
                ...
                self.info(
                    "{:10}: {}".format(
                        "zlib",
                        "PYXMLSEC_ZLIB_VERSION unset, downloading latest from {}".format(
                            url
                        ),
                    )
                )
            else:
                url = "https://zlib.net/fossils/zlib-{}.tar.gz".format(
                    self.zlib_version
                )
                self.info(
                    "{:10}: {}".format(
                        "zlib",
                        "PYXMLSEC_ZLIB_VERSION={}, downloading from {}".format(
                            self.zlib_version, url
                        ),
                    )
                )
        ...
        if libiconv_tar is None:
            self.info(
                "{:10}: {}".format("libiconv", "source not found, downloading ...")
            )
            ...
            if self.libiconv_version is None:
                ...
                self.info(
                    "{:10}: {}".format(
                        "zlib",
                        "PYXMLSEC_LIBICONV_VERSION unset, downloading latest from {}".format(
                            url
                        ),
                    )
                )
            else:
                url = "https://ftp.gnu.org/pub/gnu/libiconv/libiconv-{}.tar.gz".format(
                    self.libiconv_version
                )
                self.info(
                    "{:10}: {}".format(
                        "zlib",
                        "PYXMLSEC_LIBICONV_VERSION={}, downloading from {}".format(
                            self.libiconv_version, url
                        ),
                    )
                )
        ...
        if libxml2_tar is None:
            self.info(
                "{:10}: {}".format("libxml2", "source tar not found, downloading ...")
            )
            if self.libxml2_version is None:
                ...
                self.info(
                    "{:10}: {}".format(
                        "libxml2",
                        "PYXMLSEC_LIBXML2_VERSION unset, downloading latest from {}".format(
                            url
                        ),
                    )
                )
                ...
                self.info(
                    "{:10}: {}".format(
                        "libxml2",
                        "PYXMLSEC_LIBXML2_VERSION={}, downloading from {}".format(
                            self.libxml2_version, url
                        ),
                    )
                )
        ...
        if libxslt_tar is None:
            self.info(
                "{:10}: {}".format("libxslt", "source tar not found, downloading ...")
            )
            if self.libxslt_version is None:
                ...
                self.info(
                    "{:10}: {}".format(
                        "libxslt",
                        "PYXMLSEC_LIBXSLT_VERSION unset, downloading latest from {}".format(
                            url
                        ),
                    )
                )
                ...
                self.info(
                    "{:10}: {}".format(
                        "libxslt",
                        "PYXMLSEC_LIBXSLT_VERSION={}, downloading from {}".format(
                            self.libxslt_version, url
                        ),
                    )
                )
        ...
        if xmlsec1_tar is None:
            self.info(
                "{:10}: {}".format("xmlsec1", "source tar not found, downloading ...")
            )
            if self.xmlsec1_version is None:
                ...
                self.info(
                    "{:10}: {}".format(
                        "xmlsec1",
                        "PYXMLSEC_XMLSEC1_VERSION unset, downloading latest from {}".format(
                            url
                        ),
                    )
                )
                ...
                self.info(
                    "{:10}: {}".format(
                        "xmlsec1",
                        "PYXMLSEC_XMLSEC1_VERSION={}, downloading from {}".format(
                            self.xmlsec1_version, url
                        ),
                    )
                )
        ...
        for file in (
            ...
            try:
                ...
                raise DistutilsError(
                    "Bad {} downloaded; remove it and try again.".format(file.name)
                )
        ...
        subprocess.check_output(
            ["make", "-j{}".format(multiprocessing.cpu_count() + 1), "install_sw"],
            cwd=str(openssl_dir),
            env=env,
        )
        ...
        subprocess.check_output(
            ["make", "-j{}".format(multiprocessing.cpu_count() + 1), "install"],
            cwd=str(zlib_dir),
            env=env,
        )
        ...
        ext.include_dirs.extend(
            [
                str(p.absolute())
                for p in (self.prefix_dir / "include").iterdir()
                if p.is_dir()
            ]
        )
